...
- **2026-06-25** — **Monograph chunk dedup built** (non-destructive): `kg_chunk.is_canonical`/`dedup_group_id`/`dedup_substance_id`/`dedup_product_count` (migrations `20260625120000`/`121000`, `scripts/dedup-monograph-chunks.mjs`). Per substance, near-duplicate monograph chunks across its single-ingredient products are clustered per section (Jaccard ≥ 0.9, brand/page/header noise stripped) and all but one marked `is_canonical=false`. Full pass: **173,696 chunks → 78,385 canonical (55% suppressed)** across 1,846 substances; a chunk with no near-duplicate stays canonical, so divergences are never lost (verified 0 cross-product divergences dropped). `check-interactions` now retrieves only `is_canonical` chunks. **Needs `supabase functions deploy check-interactions` to go live.**
- **2026-06-25** — **ATC class hierarchy built** (the class-layer scaffold): WHO ATC L1–L4 reference (`scripts/data/atc_classes.json`, sourced from the public WHO ATC-DDD index) → `scripts/build-atc-hierarchy.mjs` creates the L1–L4 `drug_class` nodes (770 new) and `subclass_of` edges ingredient→L4→L3→L2→L1 (2,637). Resolution works: escitalopram → SSRIs → Antidepressants → Psychoanaleptics → Nervous System; "Vitamin K antagonists" → warfarin, acenocoumarol (the Lexicomp "Interacting Members" model, now derivable). Covers the **~1,772 ingredients that carry a real ATC-5 code** (the rest have empty `atc:[]` arrays — an ingestion artifact; ATC enrichment + that cleanup are follow-ups).
- **2026-06-25** — **QT-prolongation PD layer built** (first pharmacodynamic axis): `scripts/extract-qt-risk.mjs` assessed 960 candidate substances (those with QT evidence; migration `20260625150000`) → **321 QT-prolonging drugs** classified into CredibleMeds-style tiers (82 known / 121 possible / 118 conditional; 632 "none"), as candidate `subclass_of` edges to 3 functional class nodes "QT-prolonging agents (… risk)" (source=QT_PD_LAYER, cited). Derivation `kg_qt_interaction` (migration `20260625160000`) + RPC `kg_explorer_qt_interactions`: any two QT agents → additive interaction (draft severity known×known=moderate else minor) → **51,360 derived pairs** — the Lexicomp "QT-prolonging Agents / QT-prolonging Agents" model. $30. Validated on ground truth (amiodarone/sotalol/methadone → known). All candidate; gate on published before runtime; a few over-calls + 1 NHP false-positive for the review queue.
- **2026-06-25** — **Combined PD-profile layer built** (17 additive-effect axes in one pass): `scripts/extract-pd-profile.mjs` profiled the monograph-bearing substances (candidate table `20260625170000`) → **4,675 class memberships across 1,727 drugs** (serotonin, CNS/respiratory depression, bleeding, hyperkalemia, hyponatremia, anticholinergic, hypoglycemia, hypotension, bradycardia, seizure-threshold, nephro/hepato-toxicity, myelosuppression, ototoxicity, constipation, photosensitivity), each a cited candidate `subclass_of` edge to a functional class node (source=PD_LAYER, magnitude). `kg_pd_interaction` view + RPC (`20260625180000`) derive **~971,000 additive PD pairs**. ~$135 total (ran across a mid-run Anthropic credit-exhaustion; `pd_processed` resume-tracking finished it cheaply). Validated (amitriptyline → anticholinergic/hypotension/seizure/sedation/serotonin/constipation/hepatotoxicity). All candidate; gate on published before runtime.
- **2026-06-25** — **Ingestion-baking started** (so consolidation stops regenerating): `supabase/seed/src/normalization/canonical-ingredient.ts` derives a **source-agnostic, salt/ester-normalized** ingredient key (`canonicalMoietyKey`) — minerals/ion-umbrellas kept distinct per salt, matching the consolidation guards — plus `buildIngredientIndex` (reuse existing consolidated nodes by name/synonym) and `resolveIngredientId`. Wired into the **DPD ingest** (`health-canada/dpd.ts`) so it resolves to the canonical node instead of minting `health-canada-dpd:ingredient:<name>` duplicates. Verified: warfarin/testosterone/escitalopram/amitriptyline salt+ester forms converge; iron/calcium salts stay distinct. **Follow-up: the same one-line `resolveIngredientId` swap into NOC / CPS / summary-report ingests** (DPD is the largest source; done first as the reference).
- **2026-06-25** — **Monograph-ingestion gap diagnosed**: of 1,993 interaction-relevant drugs, **188 lack monograph TEXT** (only DPD/product-listing chunks) — list in `out/monograph-gap-drugs.txt`. It's a content gap, not linkage (e.g. warfarin's CPS nodes exist but carry only `product_listing` chunks). ~60–80 are real current Canadian drugs (alprazolam, chlorpromazine, cimetidine, amitriptyline, asenapine…); the rest are off-market (astemizole, cisapride, cerivastatin), research chemicals (aniline, CCl₄), or NHPs that legitimately have no monograph. **Fix: targeted monograph re-ingest** for the real set — CPS CaaS export→ingest (`cps:caas:export` / `cps:ingest`, API key configured) or HC product-monograph ingest at un-processed DPD offsets. Scoped, not yet run (heavy/API).
- **Open (Lexicomp-informed) workstreams**: hybrid (chosen) — ATC hierarchy (done); QT + 17-axis PD layers (done); the mechanism→class generalization rule (amiodarone→all VKAs vs only simvastatin); ATC-coverage enrichment for ingredients lacking codes; curated functional class membership (e.g. QT-prolongation Moderate / Indeterminate-Avoid), the "interacting members" class-interaction model; mechanism→class generalization rule (amiodarone→all VKAs vs only simvastatin); bake canonical-resolution into ingestion so duplication stops regenerating; retrieval diversity-rank/cap on top of `is_canonical`.
