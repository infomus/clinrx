...
- **Open (Lexicomp-informed) workstreams**: curated functional class membership + PD risk-tier categories (e.g. QT-prolongation Moderate / Indeterminate-Avoid), the "interacting members" class-interaction model; mechanism→class generalization rule (amiodarone→all VKAs vs only simvastatin); bake canonical-resolution into ingestion so duplication stops regenerating; retrieval diversity-rank/cap on top of `is_canonical`.
