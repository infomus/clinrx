-- Consolidate cross-source duplicate ingredient nodes (matched-crosswalk driven,
-- small clusters only; minerals deferred). Re-home every reference from each
-- loser node onto its canonical (kg_ingredient_merge_map), dedup, then delete the
-- losers. For PK-constrained child tables we insert canonical-pointing copies
-- (on conflict do nothing, which absorbs both existing-canonical and
-- sibling-loser collisions) and then delete the loser rows.
