-- Ester consolidation: collapse ester-named substance duplicates to the moiety
-- (testosterone enanthate/cypionate -> testosterone, estradiol valerate ->
-- estradiol, etc.). Esters added to the salt-strip list in the merge-map builder;
-- consistent with the "salts/esters/hydrates collapse to base moiety" policy
-- (route/duration lives on the product nodes). Standard re-homing from
-- kg_ingredient_merge_map.
