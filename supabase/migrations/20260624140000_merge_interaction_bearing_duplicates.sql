-- Consolidate the INTERACTION-BEARING duplicate ingredient nodes (name-primary,
-- ATC guardrail, minerals/vitamins + combination products excluded). The scoped
-- loser->canonical map is in kg_ingredient_merge_map (built by the interaction-
-- bearing merge-map builder). Re-home every reference from each loser onto its
-- canonical, dedup, then delete the losers. Mirrors the matched-crosswalk merge
-- (20260618232000) with one addition: preserve a loser's ATC onto a canonical
-- that lacks it (conflicting-ATC clusters were excluded upstream).
