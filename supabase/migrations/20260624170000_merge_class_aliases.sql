-- Step 3 of the spine consolidation: merge true drug_class alias duplicates only.
-- Of 38 exact-name class clusters, only same-/no-ATC ones are real duplicates
-- (e.g. a CLINRX functional-class alias + the CPS class). Multi-ATC name clusters
-- (a substance classified in several ATC categories) and generic "COMBINATIONS"
-- labels are NOT duplicates and are excluded by the map builder. Map (loser class
-- node -> canonical class node) in kg_ingredient_merge_map; standard re-homing.
